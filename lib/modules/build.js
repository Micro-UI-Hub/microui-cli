...
const collectReadmeFiles = function (filePath, targetPath) {
    ...
    let fullpath = targetPath + tPath;
    cache.files.push(fullpath);
    ncp(filePath, fullpath, function (err) {
        if (err) {
...
const fnBuild = function (a) {
    ...
    try {
        ...
        if (manifest.readme) {
            ...
            let endPath = collectReadmeFiles(manifest.readme, targetPath);
            manifest.readme = endPath;
        }

        if (manifest.components && manifest.components.length) {
            manifest.components = manifest.components.map(item => {
                let endPath = collectReadmeFiles(item.doc, targetPath);
                item.doc = endPath;
                return item;
            });
        }
